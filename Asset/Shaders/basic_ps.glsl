...
void main(void)
{
    ...
    L = normalize(L);
    vec3 light_dir = normalize((viewMatrix * worldMatrix * vec4(normalize(lightDirection),0.0f)).xyz);
    ...
    if (diffuseColor.r < 0)
        outputColor = vec4(ambientColor.rgb + lightIntensity * atten * lightColor.rgb * texture(defaultSampler, uv).rgb * clamp(dot(N, L), 0.0f, 1.0f) + specularColor.rgb * pow(clamp(dot(R, V), 0.0f, 1.0f), specularPower), 1.0f); 
    else
