...
namespace Aurora
{
	...
	{
	public:
		Engine(std::unique_ptr<GraphicsManager>&& graphics_manager, std::unique_ptr<IShaderManager>&& shader_manager, std::unique_ptr<SceneManager>&& scene_manager, std::unique_ptr<InputManager>&& input_manager, std::unique_ptr<IPhysicsManager>&& physics_manager);
		...
		const std::unique_ptr<IShaderManager>& GetShaderManager() const { return shader_manager_; }
	...
	private:
		...
		std::unique_ptr<IShaderManager> shader_manager_;
