...
namespace Aurora
{
	...
	{
	public:
		Engine(std::unique_ptr<GraphicsManager>&& graphics_manager, std::unique_ptr<ShaderManager>&& shader_manager, std::unique_ptr<SceneManager>&& scene_manager, std::unique_ptr<InputManager>&& input_manager, std::unique_ptr<IPhysicsManager>&& physics_manager);
		...
		const std::unique_ptr<ShaderManager>& GetShaderManager() const { return shader_manager_; }
